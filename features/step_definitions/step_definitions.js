...
var StepDefsWrapper = function (callback) {
  ...
  this.Given(/^a number of related, but unmatched identity records exist:$/, function (callback) {
    //this.connect(callback);
  ...
  this.Given(/^existing identity records have associated events:$/, function (callback) {
    //this.connect(callback);
  ...
  this.When(/^a matchable update is received for that identity:$/, function (callback) {
    //this.connect(callback);
...
module.exports = StepDefsWrapper;